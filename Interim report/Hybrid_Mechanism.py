...
def pm_scalar_norm(t: float, epsilon: float, rng: Optional[np.random.Generator] = None) -> float:
    if rng is None:
    ...
    return float(noisy_t)

def duchi_scalar_norm(t: float, epsilon: float, rng: Optional[np.random.Generator] = None) -> float:
    """Duchi et al.'s 1-D mechanism on normalized t ∈ [-1,1]: returns ±B with linear probs; unbiased."""
    if rng is None:
        rng = np.random.default_rng()
    e = np.exp(epsilon)
    B = (e + 1.0) / (e - 1.0)
    p_pos = ((e - 1.0) / (2.0 * e + 2.0)) * t + 0.5
    u = rng.random()
    sign = +1.0 if u < p_pos else -1.0
    return float(sign * B)

# ---------------- HM wrapper ---------------- #

def hm_scalar(x: float, epsilon: float, L: float, U: float, rng: Optional[np.random.Generator] = None) -> float:
    if rng is None:
        rng = np.random.default_rng()
    if U <= L:
    ...
    eps_star = 0.61  # per Lemma 3 (approx)
    if epsilon <= eps_star:
        alpha = 0.0
    else:
        alpha = 1.0 - np.exp(-epsilon / 2.0)
        alpha = float(np.clip(alpha, 0.0, 1.0))
    u = rng.random()
    if u < alpha:
        noisy_t = pm_scalar_norm(t, epsilon, rng=rng)
    else:
        noisy_t = duchi_scalar_norm(t, epsilon, rng=rng)
...
def run(values: np.ndarray, epsilon: float, L: float, U: float, seed: Optional[int]) -> dict:
    ...
    noisy_vals = np.array([hm_scalar(x, epsilon, L, U, rng=rng) for x in values], dtype=float)
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Hybrid Mechanism (HM) under LDP for a single CSV column.")
